...
export default function RandomButton({ input, randomHovers, error }) {
    if (!randomHovers[input]) {
        return (
            <svg
                className={`random random--empty ${error ? " random--error" : ""}`}
        ...
        return (
            <svg
                className={`random random--filled ${error ? " random--error" : ""}`}
                xmlns="http://www.w3.org/2000/svg"
                viewBox="1 0.5 48 48"
            >
